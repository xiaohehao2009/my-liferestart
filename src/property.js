...
class Property {
    ...
    effect(effects) {
        for(let prop in effects) {
            if (prop === this.TYPES.ALL) {
                for (const [condition, effect] of effects[prop]) {
                    const arr = condition.split('_');
                    const obj = {};
                    for (const type of this.SPECIAL.RDM) {
                        if (checkCondition(this, arr.join(type))) {
                            obj[type] = effect;
                        }
                    }
                    this.effect(obj);
            ...
            const value = String(effects[prop]);
            const hookedProp = this.hookSpecial(prop);
            if (value.startsWith('=')) {
                this.hl(this.#data[hookedProp] = Number.parseInt(value.slice(1)));
                continue;
            }
            if (value.startsWith('*')) {
                this.hl(this.#data[hookedProp] *= Number.parseInt(value.slice(1)));
            ...
            this.change(hookedProp, Number(value));
        }
    }
