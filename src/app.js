...
class App{
    ...
    initPages() {
        ...
        // Loading
        const loadingPage = $(
            '<div id="main"><div id="title">人生重开模拟器<br>' +
            '<div style="font-size:1.5rem; font-weight:normal;">加载中...</div>' +
            '</div></div>');

        // Index
        const indexPage = $(
            '<div id="main"><button id="achievement">成就</button><button id="specialthanks">特别感谢</button>' +
            '<button id="themeToggleBtn">黑</button><button id="save">Save</button><button id="load">Load</button><div id="title">' +
            '人生重开模拟器<br><div style="font-size:1.5rem; font-weight:normal;">这垃圾人生一秒也不想呆了</div></div>' +
        ...
        const specialThanksPage = $(
            '<div id="main"><button id="specialthanks">返回</button><div id="spthx"><ul class="g1"></ul><ul class="g2"></ul>' +
            `</div><button class="sponsor" onclick="globalThis.open('https://afdian.net/@LifeRestart')" style="background:` +
            'linear-gradient(90deg,#946ce6,#7e5fd9);left:auto;right:50%;transform:translate(-2rem,-50%)">打赏策划(爱发电)</button>' +
            `<button class="sponsor" onclick="globalThis.open('https://dun.mianbaoduo.com/@vickscarlet')" style="background-color:#c69;` +
            'left:50%;right:auto;transform:translate(2rem,-50%)">打赏程序(顿顿饭)</button></div>');
        ...
        const achievementPage = $(
            '<div id="main"><button id="specialthanks">返回</button><span class="title">统计</span><ul id="total"></ul>' +
            '<span style="padding:0.25rem; margin: 0.5rem 0; border: none; background: #ccc;"></span><span class="title">' +
            '成就<button id="rank">排行榜</button></span><ul id="achievements"></ul>'
        )
        ...
        // Talent
        const talentPage = $(
            '<div id="main"><div class="head" style="font-size:1.6rem">天赋抽卡</div>' +
            '<button id="random" class="mainbtn" style="position:fixed;top:50%;left:50%;transform:translate(-50%, -50%)">抽一抽！</button>' +
            '<ul id="talents" class="selectlist" style="overflow:auto"></ul><button id="next" class="mainbtn">请选择</button></div>'
        );

        const createTalent = ({ grade, name, description }) => {
            return $(`<li class="grade${grade}b">${name}${description ? `（${description}）` : ''}</li>`);
        };
                    ...
                    .forEach(talent=>{
                        ...
                        li.click(()=>{
                            if(li.hasClass('selected')) {
                                const selected = Array.from(this.#talentSelected).map(({id})=>id);
                                const requires = this.#life.findRequire(selected, talent.id);
                                if (requires) {
                                    this.hint(`此天赋被天赋【${requires.map(s=>s.name).join('、')}】需要`);
                                    return;
                                }
                                li.removeClass('selected');
                                this.#talentSelected.delete(talent);
                                if(this.#talentSelected.size<1) {
                                    talentPage.find('#next').text('请选择');
                                }
                            } else {
                                const selected = Array.from(this.#talentSelected).map(({id})=>id);
                                ...
                                const require = this.#life.require(
                                    selected,
                                    talent.id
                                );
                                if(exclusive != null) {
                                ...
                                if(require != null) {
                                    this.hint(`需要天赋【${require.map(s=>s.name).join('、')}】`);
                                    return;
                                }
        ...
        talentPage
            ...
            .click(()=>{
                ...
                this.#totalMax = 1000000000 + this.#life.getTalentAllocationAddition(Array.from(this.#talentSelected).map(({id})=>id));
        ...
        const propertyPage = $(/*html*/
            '<div id="main"><div class="head" style="font-size:1.6rem"><div>调整初始属性</div>' +
            '<div id="total" style="font-size:1rem;font-weight:normal">可用属性点：0</div></div>' +
            '<ul id="propertyAllocation" class="propinitial"></ul>' +
            '<ul class="selectlist" id="talentSelectedView" style="overflow:auto"></ul>' +
            '<div class="btn-area"><button id="random" class="mainbtn">平均分配</button>' +
            '<button id="start" class="mainbtn">开始新人生</button></div></div>'
        );
        ...
        groups.CHR = getBtnGroups("颜值"); // 颜值 charm CHR
        groups.INT = getBtnGroups("智力"); // 智力 intelligence INT
        groups.STR = getBtnGroups("体质"); // 体质 strength STR
        groups.MNY = getBtnGroups("家境"); // 家境 money MNY
        groups.SPR = getBtnGroups("快乐"); // 快乐 SPR
        ...
        propertyPage
            .find('#random')
            .click(()=>{
                const value = Math.floor(this.#totalMax / 5);
                groups.CHR.set(value);
                groups.INT.set(value);
                groups.STR.set(value);
                groups.MNY.set(value);
                groups.SPR.set(value);
            });
        ...
        const trajectoryPage = $(
            '<div id="main"><ul id="lifeProperty" class="lifeProperty"></ul>' +
            '<ul id="lifeTrajectory" class="lifeTrajectory"></ul>' +
            '<div class="btn-area"><button id="auto" class="mainbtn">自动播放</button>' +
            '<button id="auto2x" class="mainbtn">自动播放2x</button><button id="summary" class="mainbtn">人生总结</button>' +
            '<button id="domToImage" class="mainbtn">人生回放</button></div>' +
            '<div class="domToImage2wx"><img src="" id="endImage" /></div></div>'
        );
        ...
        // Summary
        const summaryPage = $(
            '<div id="main"><div class="head">人生总结</div><ul id="judge" class="judge"></ul>' +
            '<div class="head" style="height:auto;">天赋，你可以选一个，下辈子还能抽到</div>' +
            '<ul id="talents" class="selectlist" style="flex:0 1 auto"></ul>' +
            '<button id="again" class="mainbtn"><span class="iconfont">&#xe6a7;</span>再次重开</button></div>'
        );
                ...
                clear: () => {
                    ...
                    this.#specialthanks
                        ...
                        .forEach(({group, name, comment, color}) => groups[--group].append(
                            `<li><span class="name" ${color?('style="color:'+color+'"'):''}>${name}</span>` +
                            `<span class="comment">${comment || ''}</span></li>`
                        ))
                }
            },
            achievement: {
    ...
    switch(page) {
        if (page === 'index' || page === 'loading') $('#indexbtn').hide();
        else $('#indexbtn').show();
